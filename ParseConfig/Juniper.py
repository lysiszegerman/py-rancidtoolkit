...
def sectionRecursive(configtree, section):
  """parse configtree recursively and return config with the desired matches in dict section
  """
  ret = dict()
...
def filterSection(configtree, filter):
  """filters configtree according to regexp terms in filter and outputs only those parts of section that
     contain values
  """
...
def filterSectionRecursive(configtree, filter):
  """filters configtree according to regexp terms in filter and outputs a dict of all matched entries
  """
  ret = dict()
  branchkeys = configtree.keys()
  for key in branchkeys:
    if type(configtree[key]) == dict:
      ...
      if re.search(filter, key):
        # if the current key matches the filter, append remaining configtree to return variable
...
def interfaces(filename):
  """find interfaces and matching descriptions from filename and returns a
     dict interface=>description
  """
  inttree = filterSection(section(filename,["interfaces"]), "description .*")
  ret = dict()
  ...
  for interface in inttree.keys():
    for unit in inttree[interface].keys():
      if not re.match("inactive: ", unit):
        ...
        if unittree.has_key("family inet"):
        ...
        # family inet
        if unittree.has_key("family inet6"):
